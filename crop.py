...
def load_equirectangular_image(equi_img_path):
    """Load and prepare equirectangular image."""
    equi_img = cv2.imread(equi_img_path)
    equi_img = cv2.cvtColor(equi_img, cv2.COLOR_BGR2RGB)
    original_equi_img = equi_img.copy()
    equi_img = np.transpose(equi_img, (2, 0, 1))
    return equi_img, original_equi_img
...
def generate_perspective_view(equi_img, rots, height, width, fov_x):
    """Generate perspective view from equirectangular image."""
...
def create_animation(input_path, output_dir, fps=30, width=640, height=None, 
    ...
    """Create animation from equirectangular image."""
    ...
    equi_img, _ = load_equirectangular_image(input_path)
...
def create_video(output_dir, fps, width, height, gif_width, gif_height):
    """Create MP4 and optimized GIF from frames."""
